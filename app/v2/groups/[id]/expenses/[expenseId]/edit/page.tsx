...
import { hueFromName, initials } from '@/lib/v2-format';

export default function V2EditExpensePage() {
  ...
  useEffect(() => {
    if (!expense || initialized.current) return;
  ...
  const updateExpense = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/v2/groups/${groupId}/expenses/${expenseId}`);
    },
  ...
  return (
    <div className="v2-enter mx-auto max-w-2xl space-y-6">
      <Link
        href={`/v2/groups/${groupId}/expenses/${expenseId}`}
      ...
      <header>
        <p className="v2-section-label">Modifica spesa</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {updateExpense.isPending ? 'Salvataggio…' : 'Salva modifiche'}
        </button>
      </form>
    </div>
  );
